...
> **Provenance.** Salvaged from PR #10 (2026-05-05), which was closed because
> its
> two code changes fixed problems that no longer exist. These operational notes
> did not go stale with them — each is a trap someone hit on a real deployment.
> Status checked 2026-08-21; where a note is now handled automatically, it
> says so.
...
> **Platform-dependent — tested 2026-08-20.** On macOS this does not reproduce:
> Docker Desktop ignores uid mapping on bind mounts, so the directory is
> writable
> whether or not it was pre-created. A control was run (the case expected to
> fail
> passed too), so `logs/.gitkeep` fixes nothing there. On **Linux**, where uid
> mapping is real, a root-created `logs/` genuinely can be unwritable to the
> container's uid — untested, so treat it as plausible rather than established.
...
> **Confirmed 2026-08-21, and it cost 15 days.** The `deploy-sidecar` job in
> `deploy.yml` reported SUCCESS on every release from 2026-08-05 onward while
> recreating no container at all. Cause: the sidecar's compose declared `build:`
> with no `image:`, so the job's `export API_IMAGE` had nothing to interpolate
> into — compose reused the locally-built image and exited 0. The gate was
> structurally incapable of failing, and two production releases passed through
> it. Fix: `image: ${API_IMAGE:-...}` alongside `build:`. **If a deploy step can
> pass without the artefact changing, it is not a gate.**
